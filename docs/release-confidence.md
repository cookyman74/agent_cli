...
- **Command:** `npx -y @didim/agent-cli@<tag> --version` must return the correct
...
- **Setup:**
  - [ ] Uninstall any existing global version:
        `npm uninstall -g @didim/agent-cli`
  - [ ] Clear npx cache (optional but recommended): `npm cache clean --force`
  - [ ] Install the preview version: `npm install -g @didim/agent-cli@preview`
