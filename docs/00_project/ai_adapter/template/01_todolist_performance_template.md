...
> - [100_Python_Performance_Guide.md](../100_Python_Performance_Guide.md) - 성능
>   가이드  
>   **작업 결과서 템플릿**:
>   [template.md](./working_history/{버전}/template.md)  
>   **버전**: v2.1 (TDD + Performance + 2일 단위 분할)
>
> **작업 분할 규칙 (필수)**:
>
...
> 아래 표를 먼저 작성한 뒤 Phase 본문을 채웁니다.

| Phase ID | 목표/범위 | 예상 소요(일) | 분할 필요 여부 | 선행 Phase | 산출물(결과서) |
| -------- | --------- | ------------- | -------------- | ---------- | -------------- |
| P1       | [범위]    | 1~2           | N              | -          | [링크]         |
| P2       | [범위]    | 1~2           | N              | P1         | [링크]         |
| P3       | [범위]    | 1~2           | Y/N            | P2         | [링크]         |
...
3. 성능 최적화는 해당 Phase의 변경분에 대해서만 수행한다.
4. 대규모 리스크 항목은 별도 Phase로 분리한다.
...
## 🔄 Phase 1: [Phase 제목 - 최대 2일]

> 📄 **상세 참고**: [detail_prd/step01_xxx.md](./detail_prd/step01_xxx.md) (있는
> 경우)

### 1.1 사전 작업 (Pre-Work)

> **목적**: 본작업의 실패를 줄이기 위한 작업 준비 과정  
> **원칙**: 전체 작업의 목적과 배경, 이전 작업결과서를 참고하여 작업의 맥락 이해
> 및 일관성 유지
...
- [ ] **[REVIEW]** 이전 작업 결과서 검토 _(Phase 2 이후)_
...
- [ ] **[ANALYSIS]** 현재 코드/설정 분석
  - 파일: `[분석 대상 파일]`
  - 확인 내용: [분석할 내용]
  - 예상 데이터 규모: [항목 수, 빈도] _(성능 민감 시)_

- [ ] **[SCOPE-CHECK]** 2일 이내 완료 가능 범위 확인
  - 예상 총 소요: [X시간 또는 X일]
  - 2일 초과 시 분할안: `Phase1-A`, `Phase1-B` 등으로 재정의
  - 이번 Phase 완료 조건(DoD): [테스트/기능/문서 기준]
...
> **원칙**: "Make it right, Make it fast" - 테스트가 통과하는 상태에서만
> 리팩터링
...
> **참고**:
> [100_Python_Performance_Guide.md](../100_Python_Performance_Guide.md)  
...
## 🔄 Phase 2: [Phase 제목 - 최대 2일]

> 📄 **상세 참고**: [detail_prd/step02_xxx.md](./detail_prd/step02_xxx.md) (있는
> 경우)
...
- [ ] **[SCOPE-CHECK]** 2일 이내 완료 가능 범위 확인
  - 예상 총 소요: [X시간 또는 X일]
  - 2일 초과 시 분할안: `Phase2-A`, `Phase2-B` 등
...
- [ ] **[REFACTOR-STRUCTURE]** 코드 구조 개선
- [ ] **[REFACTOR-PERF-ANALYZE]** 성능 체크리스트 검토 _(성능 민감 시)_
- [ ] **[REFACTOR-PERF-OPTIMIZE]** 성능 최적화 적용 _(해당 시)_
...
## 🔄 Phase N: [Phase 제목 - 최대 2일]

_(필요한 만큼 Phase 추가 - 동일 구조, 각 Phase는 2일 이내로 제한)_
...
9. **2일 제한 준수**: Phase는 반드시 2일 이내 완료 가능한 단위로 유지
10. **커밋 단위**: Phase 단위로 커밋하여 롤백 용이하게 관리
11. **린터 통과**: 커밋 전 반드시 린터 검사 통과
...
- [100_Python_Performance_Guide.md](../100_Python_Performance_Guide.md) - 파이썬
  성능 가이드 (Refactor 참조)
