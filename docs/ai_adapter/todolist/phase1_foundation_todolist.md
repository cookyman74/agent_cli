...
Always follow TDD principles. For each task: write a failing test first, implement minimum code to pass, then refactor. Never mix structural and behavioral changes in the same commit.
...
- [ ] 이전 작업 리뷰: 이전 Milestone 작업 결과서 확인 (`working_history/` 디렉토리)
...
| ID | 작업 | 상태 | 테스트 |
|----|------|------|--------|
| 1.0.1.1 | `@google/genai` import 사용 파일 전체 스캔 | ⬜ | N/A (분석) |
| 1.0.1.2 | 직접 타입 참조 vs 간접 참조 분류 | ⬜ | N/A (분석) |
| 1.0.1.3 | 핵심 경로(critical path) 파일 식별 | ⬜ | N/A (분석) |
| 1.0.1.4 | 의존성 그래프 시각화 문서 작성 | ⬜ | N/A (문서) |
...
| ID | 작업 | 상태 | 테스트 |
|----|------|------|--------|
| 1.0.2.1 | GeminiEventType 18개 이벤트 사용처 분석 | ⬜ | N/A (분석) |
| 1.0.2.2 | ServerGeminiStreamEvent 소비자 식별 | ⬜ | N/A (분석) |
| 1.0.2.3 | 이벤트 흐름 다이어그램 작성 | ⬜ | N/A (문서) |
| 1.0.2.4 | 프로바이더별 이벤트 매핑 초안 작성 | ⬜ | N/A (문서) |
...
| ID | 작업 | 상태 | 테스트 |
|----|------|------|--------|
| 1.0.3.1 | `ContentGenerator` 의존 파일 분석 | ⬜ | N/A (분석) |
| 1.0.3.2 | `StreamEvent`/`StreamEventType` 사용처 분석 | ⬜ | N/A (분석) |
| 1.0.3.3 | Hook 시스템 의존 분석 | ⬜ | N/A (분석) |
| 1.0.3.4 | Telemetry 의존 분석 | ⬜ | N/A (분석) |
| 1.0.3.5 | Retry 로직 의존 분석 | ⬜ | N/A (분석) |

### 1.0.4 🆕 ModelConfigService 종속성 분석
| ID | 작업 | 상태 | 테스트 |
|----|------|------|--------|
| 1.0.4.1 | `GenerateContentConfig` 사용처 전수 조사 | ⬜ | N/A (분석) |
| 1.0.4.2 | `ModelRouterService` 연동 지점 식별 | ⬜ | N/A (분석) |
| 1.0.4.3 | 설정 우선순위/머지 로직 분석 | ⬜ | N/A (분석) |
| 1.0.4.4 | 호환 레이어 설계안 작성 | ⬜ | N/A (설계) |

### 1.0.5 마이그레이션 전략 수립
| ID | 작업 | 상태 | 테스트 |
|----|------|------|--------|
| 1.0.5.1 | 단계별 마이그레이션 순서 정의 | ⬜ | N/A (설계) |
| 1.0.5.2 | alias 전략 설계 (레거시 호환) | ⬜ | N/A (설계) |
| 1.0.5.3 | 병행 운영 기간 정의 | ⬜ | N/A (설계) |
| 1.0.5.4 | migration-plan.md 문서 작성 | ⬜ | N/A (문서) |
| 1.0.5.5 | 🆕 event-mapping-matrix.md 문서 작성 | ⬜ | N/A (문서) |
...
| ID | 작업 | 상태 | 테스트 파일 |
|----|------|------|------------|
| 1.1.1.1 | `LlmRole` enum 정의 | ⬜ | `types.test.ts` |
| 1.1.1.2 | `LlmTextContent` 타입 정의 | ⬜ | `types.test.ts` |
| 1.1.1.3 | `LlmImageContent` 타입 정의 | ⬜ | `types.test.ts` |
| 1.1.1.4 | `LlmToolCallContent` 타입 정의 | ⬜ | `types.test.ts` |
| 1.1.1.5 | `LlmToolResultContent` 타입 정의 | ⬜ | `types.test.ts` |
| 1.1.1.6 | `LlmThoughtContent` 타입 정의 | ⬜ | `types.test.ts` |
| 1.1.1.7 | `LlmContent` union 타입 정의 | ⬜ | `types.test.ts` |
| 1.1.1.8 | `LlmMessage` 인터페이스 정의 | ⬜ | `types.test.ts` |

### 1.1.2 요청/응답 타입 정의
| ID | 작업 | 상태 | 테스트 파일 |
|----|------|------|------------|
| 1.1.2.1 | `LlmGenerateRequest` 인터페이스 | ⬜ | `types.test.ts` |
| 1.1.2.2 | `LlmGenerateResponse` 인터페이스 | ⬜ | `types.test.ts` |
| 1.1.2.3 | `LlmUsage` 인터페이스 | ⬜ | `types.test.ts` |
| 1.1.2.4 | `LlmToolDefinition` 인터페이스 | ⬜ | `types.test.ts` |
| 1.1.2.5 | 🆕 `LlmGenerateConfig` 인터페이스 (프로바이더 독립) | ⬜ | `types.test.ts` |

### 1.1.3 🆕 스트림 이벤트 타입 정의 (`events.ts`) - 확대
| ID | 작업 | 상태 | 테스트 파일 |
|----|------|------|------------|
| 1.1.3.1 | `LlmStreamEventType` enum (18개 매핑) | ⬜ | `events.test.ts` |
| 1.1.3.2 | `LlmStreamTextDelta` 타입 | ⬜ | `events.test.ts` |
| 1.1.3.3 | `LlmStreamToolCallDelta` 타입 | ⬜ | `events.test.ts` |
| 1.1.3.4 | `LlmStreamThoughtDelta` 타입 | ⬜ | `events.test.ts` |
| 1.1.3.5 | `LlmStreamUsage` 타입 | ⬜ | `events.test.ts` |
| 1.1.3.6 | 🆕 `LlmStreamToolCallRequest` 타입 | ⬜ | `events.test.ts` |
| 1.1.3.7 | 🆕 `LlmStreamToolCallResponse` 타입 | ⬜ | `events.test.ts` |
| 1.1.3.8 | 🆕 `LlmStreamError` 타입 | ⬜ | `events.test.ts` |
| 1.1.3.9 | 🆕 `LlmStreamFinished` 타입 | ⬜ | `events.test.ts` |
| 1.1.3.10 | 🆕 `LlmStreamRetry` 타입 | ⬜ | `events.test.ts` |
| 1.1.3.11 | 🆕 `LlmStreamCitation` 타입 | ⬜ | `events.test.ts` |
| 1.1.3.12 | `LlmStreamEvent` union 타입 | ⬜ | `events.test.ts` |
| 1.1.3.13 | `LlmStream` AsyncIterable 타입 | ⬜ | `events.test.ts` |
...
| ID | 작업 | 상태 | 테스트 파일 |
|----|------|------|------------|
| 1.1.4.1 | `LlmErrorType` enum | ⬜ | `errors.test.ts` |
| 1.1.4.2 | `LlmError` 기본 클래스 | ⬜ | `errors.test.ts` |
| 1.1.4.3 | `LlmRateLimitError` 클래스 | ⬜ | `errors.test.ts` |
| 1.1.4.4 | `LlmAuthenticationError` 클래스 | ⬜ | `errors.test.ts` |
| 1.1.4.5 | `LlmInvalidRequestError` 클래스 | ⬜ | `errors.test.ts` |
| 1.1.4.6 | `LlmStreamError` 클래스 | ⬜ | `errors.test.ts` |
| 1.1.4.7 | 에러 변환 유틸 함수 | ⬜ | `errors.test.ts` |

### 1.1.5 레거시 호환 레이어 (`legacyAliases.ts`)
| ID | 작업 | 상태 | 테스트 파일 |
|----|------|------|------------|
| 1.1.5.1 | `GenerateContentResponse` alias | ⬜ | `legacyAliases.test.ts` |
| 1.1.5.2 | `StreamEvent` alias | ⬜ | `legacyAliases.test.ts` |
| 1.1.5.3 | `Content` alias | ⬜ | `legacyAliases.test.ts` |
| 1.1.5.4 | `GeminiEventType` alias (기존 유지) | ⬜ | `legacyAliases.test.ts` |
| 1.1.5.5 | 변환 헬퍼 함수 | ⬜ | `legacyAliases.test.ts` |
...
- [ ] 🆕 ModelConfigService와 호환 레이어 동작 확인
