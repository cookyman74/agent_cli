...
compiler option inheritance). Running `tsc` at the root will emit `.js`,
`.d.ts`, and `.js.map` files alongside every `.ts` source file (~4,500
artifacts), polluting the source directories. Always use the npm scripts below
or run `tsc --build` from within a specific package directory.
