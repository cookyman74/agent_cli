...
Always follow the instructions in this plan. When I say "go", find the next
unmarked task in the current phase, implement the test, then implement only
...
Gemini CLI를 어댑터 패턴으로 리팩토링하여 다중 LLM 프로바이더(Gemini, Claude,
OpenAI, vLLM 등)를 지원
...
- `packages/core/src/core/contentGenerator.ts` - `GenerateContentParameters`,
  `GenerateContentResponse`
- `packages/core/src/core/baseLlmClient.ts` - `Content`, `Part`,
  `GenerateContentConfig`
- `packages/core/src/core/turn.ts` - `PartListUnion`, `FunctionCall`,
  `FinishReason`
...
모든 마일스톤 작업은 다음 3단계 프로세스로 진행:

```
┌─────────────────────────────────────────────────────────────────────────┐
│  1️⃣ 사전작업 (Pre-work)                                                 │
│  ├── 작업 개요 파악: 현재 마일스톤 목표 및 세부 작업 확인               │
│  ├── 이전 작업 리뷰: 이전 마일스톤의 작업 결과서 확인                   │
│  ├── 이슈 파악: 이전 작업에서 전달된 이슈 및 Open Questions 확인        │
│  └── 작업 일관성 검증: 설계서/마스터플랜과의 정합성 확인                │
│                                                                         │
...
│                                                                         │
│  3️⃣ 사후작업 (Post-work)                                                │
│  ├── 체크리스트 최종 확인: 해당 마일스톤 모든 항목 완료 확인            │
│  ├── 작업 결과서 작성: working_history/{단계}_{타이틀}_{날짜}.md        │
...
```
docs/ai_adapter/working_history/{Phase}_{Milestone}_{작업일자}.md

예시:
- Phase1_M1.0_코드인벤토리_20260201.md
- Phase2_M2.1_타입전환_20260210.md
```

## 작업 결과서 템플릿

- 경로: `docs/ai_adapter/template/03_work_result_report_template.md`
- 필수 섹션: 작업 개요, Red Phase, Green Phase, 검증 결과, 이슈 및 해결, 다음
  단계, 커밋 요약
