...
export class Logger {
  private projectTempDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.projectTempDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.projectTempDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.projectTempDir, { recursive: true });
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.projectTempDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.projectTempDir) {
      debugLogger.error(
