...
> level will generate ~4,500 build artifacts (`.js`, `.d.ts`, `.js.map`)
> directly alongside your source files, polluting the entire source tree. Always
> use `npm run build` or run `tsc --build` from within a specific package
> directory (e.g., `packages/core/`).
