...
`npm install -g @didim365/agent-cli@latest`. If you compiled it from source,
...
- **Use a `.env` file:** Create a `.env` file in your project's `.didim`
  directory (`.didim/.env`) and store your keys there. Gemini CLI will
...
1.  In your home directory: `~/.didim/settings.json`.
2.  In your project's root directory: `./.didim/settings.json`.
