...
- [ ] Legacy alias 설계 (`type LegacyGenerateContentResponse = ...`)로 점진적
      이행
...
**예시 시나리오**: | 설정 | 결과 | |------|------| | `LLM_PROVIDER=claude` +
`ANTHROPIC_API_KEY=xxx` | Claude 사용 | | `LLM_PROVIDER=didim` +
`DIDIM_API_KEY=xxx` | DidimAIStudio 사용 | | `LLM_PROVIDER` 없음 +
`authType=USE_GEMINI` | 기존 Gemini API Key 모드 | | `LLM_PROVIDER` 없음 +
`authType=LOGIN_WITH_GOOGLE` | 기존 OAuth 모드 | | 모두 없음 +
`GEMINI_API_KEY=xxx` | Gemini API Key 모드 (자동 감지) |
