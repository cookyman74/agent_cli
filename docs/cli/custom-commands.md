...
1.  **User commands (global):** Located in `~/.didim/commands/`. These commands
    are available in any project you are working on.
2.  **Project commands (local):** Located in
    `<your-project-root>/.didim/commands/`. These commands are specific to the
...
- A file at `~/.didim/commands/test.toml` becomes the command `/test`.
- A file at `<project>/.didim/commands/git/commit.toml` becomes the namespaced
  command `/git:commit`.
...
```toml
# In: <project>/.didim/commands/changelog.toml
...
````toml
# In: <project>/.didim/commands/git/commit.toml
...
- **Directory listing**: `@{path/to/dir}` is traversed and each file present
  ...
  respects `.gitignore` and `.didimignore` if enabled.
...
```toml
# In: <project>/.didim/commands/review.toml
...
```bash
mkdir -p ~/.didim/commands/refactor
touch ~/.didim/commands/refactor/pure.toml
```

**2. Add the content to the file:**

Open `~/.didim/commands/refactor/pure.toml` in your editor and add the following
content. We are including the optional `description` for best practice.

```toml
# In: ~/.didim/commands/refactor/pure.toml
